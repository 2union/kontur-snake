...
class MAIN:
    def __init__(self, cell_size, cell_number, info_cells, screen):
        ...
        self.net = Client(config.SOCKET_POINT, config.PACKAGE_SIZE)
        self.name_team_one = ''
        ...
        self.timer = GameTimer(config.SESSION_LENGTH)
        self.goals = [0, 0]
        self.last_goals = [0, 0]
    ...
    def draw_team(self, cells_padding, window_width, score, name):
        ...
        team_rect = team_surface.get_rect(center=(window_width // 2, self.cell_size * (cells_padding + 0.5)))
        screen.blit(team_surface, team_rect)

    def draw_waiting_screen(self):
        window_width = self.cell_size * (self.info_cells + self.cell_number)
        window_height = self.cell_size * self.cell_number
        pygame.draw.rect(self.screen, (0, 0, 0),
                         pygame.Rect(0, 0, window_width, window_height))
        rect = pygame.Rect(window_width // 4, window_height // 4, self.cell_size, self.cell_size)
        self.screen.blit(self.resources.wait, rect)

        score_surface = self.resources.score_font.render(f'{self.last_goals[0]}:{self.last_goals[1]}', True,
                                                         (255, 255, 255))
    ...
    def draw_player(self, cells_padding, direction, name):
        ...
        resized = pygame.transform.scale(resource, (self.cell_size, self.cell_size))

        rect = pygame.Rect(self.cell_size * (self.cell_number + 2), self.cell_size * cells_padding, self.cell_size, self.cell_size)
        self.screen.blit(resized, rect)

        player_surface = self.resources.player_font.render(name, True, (255, 255, 255))
        player_rect = player_surface.get_rect(center=(self.cell_size * (self.cell_number + 6),
                                                      self.cell_size * (cells_padding + 0.5)))
        self.screen.blit(player_surface, player_rect)

    def draw_scoreboard(self):
        pygame.draw.rect(self.screen, (0, 0, 0),
                         pygame.Rect(self.cell_size * self.cell_number, 0, self.cell_size * self.info_cells,
                                     self.cell_size * self.cell_number))
        kontur_logo_rect = pygame.Rect(self.cell_size * self.cell_number + self.cell_size / 2,
                                       self.cell_size / 2, self.cell_size * self.info_cells, self.cell_size * 3)
        kontur_logo_resized = self.resources.kontur_logo
        self.screen.blit(kontur_logo_resized, kontur_logo_rect)

        snake_surface = self.resources.snake_font.render("Змейка", True, (255, 255, 255))
        snake_rect = snake_surface.get_rect(center=(self.cell_size * (self.cell_number + 7.3), self.cell_size * 3.3))
        screen.blit(snake_surface, snake_rect)

        directions = ['up', 'down', 'right', 'left']
        # first team
        pygame.draw.rect(screen, (255, 211, 66),
                         pygame.Rect(self.cell_size * (self.cell_number + 1), self.cell_size * 4,
                                     self.cell_size * (self.info_cells - 2),
                                     self.cell_size * (self.cell_number - 6) / 2))
        pygame.draw.rect(screen, (0, 0, 0),
                         pygame.Rect(self.cell_size * (self.cell_number + 1) + 4, self.cell_size * 4 + 4,
                                     self.cell_size * (self.info_cells - 2) - 8,
                                     self.cell_size * (self.cell_number - 6) / 2 - 8))

        team_surface = self.resources.team_font.render(self.name_team_one, True, (255, 211, 66))
        team_rect = team_surface.get_rect(center=(self.cell_size * (self.cell_number + 5.2), self.cell_size * 5))
        ...
        # second team
        pygame.draw.rect(self.screen, (91, 123, 249),
                         pygame.Rect(self.cell_size * (self.cell_number + 1),
                                     self.cell_size * (self.cell_number / 2 + 2),
                                     self.cell_size * (self.info_cells - 2),
                                     self.cell_size * (self.cell_number - 6) / 2))
        pygame.draw.rect(self.screen, (0, 0, 0),
                         pygame.Rect(self.cell_size * (self.cell_number + 1) + 4,
                                     self.cell_size * (self.cell_number / 2 + 2) + 4,
                                     self.cell_size * (self.info_cells - 2) - 8,
                                     self.cell_size * (self.cell_number - 6) / 2 - 8))

        team_two_surface = self.resources.team_font.render(self.name_team_two, True, (91, 123, 249))
        team_two_rect = team_two_surface.get_rect(center=(self.cell_size * (self.cell_number + 5.2),
                                                          self.cell_size * 16))
        self.screen.blit(team_two_surface, team_two_rect)
    ...
    def draw_score(self):
        ...
        score_x = int(self.cell_size * self.cell_number + self.cell_size * self.info_cells - self.cell_size * 2)
        score_y = int(self.cell_size * (self.cell_number / 2 + 1))
        ...
        self.screen.blit(score_surface, score_rect)
        self.screen.blit(self.resources.apple, apple_rect)
    ...
    def draw_score_second(self):
        ...
        score_surface = self.resources.game_font.render(score_text, True, (56, 74, 12))
        score_x = int(self.cell_size * self.cell_number + self.cell_size * self.info_cells - self.cell_size * 2)
        score_y = int(self.cell_size * self.cell_number - self.cell_size)
        score_rect = score_surface.get_rect(center=(score_x, score_y))
        apple_rect = self.resources.apple.get_rect(midright=(score_rect.left, score_rect.centery))
        bg_rect = pygame.Rect(apple_rect.left, apple_rect.top, apple_rect.width + score_rect.width + 6,
                              apple_rect.height)
    ...
    def draw_timer(self):
        ...
        score_surface = self.resources.game_font.render(score_text, True, (56, 74, 12))
        score_x = int(self.cell_size * self.cell_number - self.cell_size * 2)
        score_y = int(self.cell_size * self.cell_number - self.cell_size)
        score_rect = score_surface.get_rect(center=(score_x, score_y))
    ...
    def draw_goals(self):
        ...
        score_surface = self.resources.game_font.render(score_text, True, (56, 74, 12))
        score_x = int(self.cell_size * self.cell_number - self.cell_size * 2)
        score_y = int(self.cell_size * self.cell_number - self.cell_size * 2)
        score_rect = score_surface.get_rect(center=(score_x, score_y))
        self.screen.blit(score_surface, score_rect)
...
def game_run():
    ...
    config.SCREEN_UPDATE = pygame.USEREVENT
    pygame.time.set_timer(config.SCREEN_UPDATE, 150)

    main_game = MAIN(cell_size, cell_number, info_cells, screen)
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
            if event.type == config.SCREEN_UPDATE:
                main_game.update()
